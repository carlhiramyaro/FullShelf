...
function PairingForm({
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-bg p-6">
      <form
        className="flex w-full max-w-sm flex-col gap-4 rounded-lg border border-ink/10 p-8"
        onSubmit={(e) => {
          e.preventDefault();
      ...
      >
        ...
        <input
          className="rounded border border-ink/20 bg-bg px-3 py-2 text-center text-lg tracking-widest uppercase"
          value={code}
        ...
        <button
          ...
          className="rounded bg-primary px-4 py-2 text-on-dark disabled:opacity-50"
        >
      ...
      <div className="grid w-full max-w-md grid-cols-2 gap-4">
        {roster.map((staff) => (
          <button
            ...
            className="rounded-lg border border-ink/10 p-6 text-lg font-medium text-ink shadow-sm hover:bg-secondary/10"
          >
            {staff.name}
          </button>
        ))}
        {roster.length === 0 && (
          <p className="col-span-2 text-center text-sm text-ink/50">
            No staff set up yet.
          </p>
        )}
      </div>
          ...
          (key, i) =>
            key === "" ? (
              ...
              <button
                ...
                className="h-16 w-16 rounded-full border border-ink/10 text-xl font-medium text-ink disabled:opacity-50"
              >
                {key}
              </button>
            ),
